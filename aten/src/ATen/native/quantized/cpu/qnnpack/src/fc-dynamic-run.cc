...
void sve_hybrid_s8s32_mmla_6x4VL (
    unsigned int num_strings, const unsigned int *string_lengths, const uint8_t * A_arg,
    size_t M, size_t N, const uint8_t *B_ptr, float * output_arg, bool accumulate, uint8_t zero_point, float scale
)

{
 ...
 __asm__ __volatile__(
    ...
    //    : [ args_ptr ] "r"(&ka),
    //      [ flags ] "r"(flags),
    //      [ offsetof_B_ptr ] "I"(offsetof(KernelArgs, B_ptr)),
    //      [ offsetof_N ] "I"(offsetof(KernelArgs, N)),
    //      [ offsetof_input_initial_col ] "I"(
    //          offsetof(KernelArgs, input_initial_col)),
...
static void compute_q8gemm_dq(
  ...
  const uint8_t* packed_w = context->packed_w;
  float* c = context->c;
  ...
  uint8_t zero_point = context->quantization_params.input_zero_point;
  float scale = context->quantization_params.multipliers[0];
  unsigned int len = k_stride;
  sve_hybrid_s8s32_mmla_6x4VL(
      ...
      c + (pixel_index + mr_block_start) * c_stride + nr_block_start +
          group_index * n,
      (bool)nr_block_start, zero_point, scale);
...
enum pytorch_qnnp_status qnnpackLinearDynamic(
  ...
  //std::cout << "----->>> pthread params groups is " << groups
  //          << " outpput_size " << output_size << " mr is " << mr << " nr is "
  //          << nr << std::endl
  //          << std::flush;
  //
